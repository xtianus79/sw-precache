...
function generatePreCacheToolbox(preCacheToolbox) {
  console.log('preCacheToolbox ', preCacheToolbox);
  return preCacheToolbox.reduce(function(prev, curr) {
    console.log('CURRR PRECACHE ', curr.preLoadedArray);
    var arrayBuilder;
    arrayBuilder = util.format('\ntoolbox.precache(%j);', curr.preLoadedArray);

    console.log('we made it here lasiddufpoasudidfpoas', arrayBuilder);

    return prev + arrayBuilder;
  }, '');
}
...
function generateRuntimeCaching(runtimeCaching) {
  ...
  return runtimeCaching.reduce(function(prev, curr) {
    ...
    console.log('prev + line ', prev + line);
...
function generate(params, callback) {
  return new Promise(function(resolve, reject) {
    ...
    if (params.preCacheToolbox) {
      preCacheToolbox = generatePreCacheToolbox(params.preCacheToolbox);
      var pathToSWToolbox = require.resolve('sw-toolbox/sw-toolbox.js');
      swToolboxCode = fs.readFileSync(pathToSWToolbox, 'utf8')
      .replace('//# sourceMappingURL=sw-toolbox.js.map', '');
    ...
    fs.readFile(params.templateFilePath, 'utf8', function(error, data) {
      ...
      var populatedTemplate = template(data)({
        ...
        preCacheToolbox: preCacheToolbox,
